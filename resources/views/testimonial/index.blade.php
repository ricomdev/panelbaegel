                    ...
                    <div class="col-12">
                        <div class="card card-warning">
                            <div class="card-header">
                                <h3 class="card-title">LISTA DE TESTIMONIOS</h3>
                            </div>
                            <!-- /.card-header -->
                            <div class="card-body">
                                    ...
                                    <tbody>
                                        @foreach($testimonials as $testimonial)
                                            <tr data-widget="expandable-table" aria-expanded="false">
                                                <td>{{$testimonial->user->nombre}}</td>
                                                <td>{{$testimonial->created_at}}</td>
                                                @if($testimonial->show==1)
                                                    <td class="text-right py-0 align-middle">
                                                        <div class="btn-group btn-group-sm">
                                                            <button onclick="updatetestimonial(event,{{$testimonial->id}},{{$testimonial->show}})" class="btn btn-info">Activo</button>
                                                        </div>
                                                    </td>
                                                @endif
                                                @if($testimonial->show==2)
                                                    <td class="text-right py-0 align-middle">
                                                        <div class="btn-group btn-group-sm">
                                                            <button onclick="updatetestimonial(event,{{$testimonial->id}},{{$testimonial->show}})" class="btn btn-danger">Oculto</button>
                                                        </div>
                                                    </td>
                                                @endif
                                            </tr>
