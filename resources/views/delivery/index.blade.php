                        ...
                        <div class="card card-warning">
                            ...
                            <div class="card-body p-0">
                                <table class="table">
                                    ...
                                    <tbody>
                                    @foreach ($stateszona as $state)
                                        <tr>
                                            <td>{{$state->zona}}</td>
                                            <td>{{$state->nombre}}</td>
                                            <td>{{$state->slug}}</td>
                                            @foreach ($state->deliveries as $delivery)
                                                <td>S/. {{$delivery->precio}}</td>
                                            @endforeach
                                            <td class="text-right py-0 align-middle">
                                                <div class="btn-group btn-group-sm">
                                                    <a href="{{route('delivery.show',$state->slug)}}" class="btn btn-info mr-2"><i class="fas fa-edit"></i></a>
                                                    <!--<button href="" onclick="delivery_delete(event,'{{$state->slug}}')" class="btn btn-danger"><i class="fas fa-trash"></i></button>-->
                                                </div>
                                            </td>
                                        </tr>
                                    @endforeach
