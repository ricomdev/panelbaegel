...
if (document.getElementById('code_subcategory')) {
    function datasubcategory() {
        let codigo = document.getElementById('code_subcategory').value;

        axios.get("/datasubcategory/" + codigo).then(response => {
        ...
        btn.addEventListener("click", function (e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.delete(`/subcategory/image/${id}`)
                        .then(res => {
                            if (res.data.success) {
                                Swal.fire({ icon: "success", text: "Imagen eliminada" });
                                datasubcategory(); // 🔄 recargar lista
                            }
                        })
                        .catch(err => {
...
document.getElementById("images").addEventListener("change", function (e) {
    ...
    files.forEach(file => {
        pendingFiles.push(file); // 👈 acumulamos todas las selecciones
    ...
    e.target.value = ""; // 🔥 reset input para poder elegir más imágenes después
...
// Guardar cambios
// ==========================
function subcategory_update(e) {
    e.preventDefault();

    Swal.fire({
        text: "Desea actualizar la subcategoría?",
        icon: "warning",
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            ...
            let codigo_antiguo = document.getElementById('code_subcategory').value;
            ...
            axios.post('/updatesubcategory/' + codigo_antiguo, datos)
                .then(response => {
                    if (response.data === 1) {
                        Swal.fire({
                            ...
                            location.href = "/subcategory/edit/" + codigo;
                        });
                        pendingFiles = []; // ✅ limpiar después de guardar
                    } else {
                        Swal.fire({ text: "No se realizó la actualización correctamente!", icon: "error" });
                    }
                    document.getElementById('code_subcategory').value = codigo;
                    datasubcategory();
                })
                .catch(error => {
                    Swal.fire({ text: "Hubo un error al actualizar la subcategoría", icon: "error" });
                    console.error(error);
                })
                .finally(() => {
                    // 🔄 Restaurar botón
